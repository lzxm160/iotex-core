...
func (dao *blockDAO) putBlock(blk *block.Block) error {
	...
	if blk.Height() > topHeight {
		if err := dao.kvstore.Put(blockNS, topHeightKey, byteutil.Uint64ToBytes(blk.Height())); err != nil {
			return errors.Wrap(err, "failed to put top height")
		}
		if err := dao.kvstore.Put(blockNS, topHashKey, hash[:]); err != nil {
			return errors.Wrap(err, "failed to put top hash")
		}
	}
...
func (dao *blockDAO) deleteTipBlock() error {
	...
	// Update tip hash
	hash, err = dao.indexer.GetBlockHash(height - 1)
	if err != nil {
		return errors.Wrap(err, "failed to get tip block hash")
	}
	if err := dao.kvstore.Put(blockNS, topHashKey, hash[:]); err != nil {
...
func (dao *blockDAO) getDBFromHash(h hash.Hash256) (db.KVStore, uint64, error) {
	height, err := dao.indexer.GetBlockHeight(h)
	if err != nil {
		return nil, 0, err
	}
