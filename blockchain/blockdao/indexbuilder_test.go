...
func TestIndexer(t *testing.T) {
	...
	testIndexer := func(kvstore db.KVStore, indexer blockindex.Indexer, t *testing.T) {
		...
		blkHash, err := indexer.GetBlockHashByActionHash(t1Hash)
		require.NoError(err)
		require.Equal(blkHash1, blkHash)
		blkHash, err = indexer.GetBlockHashByActionHash(t2Hash)
		require.NoError(err)
		require.Equal(blkHash2, blkHash)
		blkHash, err = indexer.GetBlockHashByActionHash(t3Hash)
		require.NoError(err)
