...
func TestProtocol_Handle(t *testing.T) {
	testEVM := func(t *testing.T) {
		...
		ws, err := sf.NewWorkingSet()
		require.NoError(err)
		_, err = accountutil.LoadOrCreateAccount(ws, identityset.Address(0).String(), unit.ConvertIotxToRau(1000000000))
		require.NoError(err)
		...
		exes, err := bc.GetActionsFromAddress(identityset.Address(0).String())
		require.Nil(err)
...
func TestProtocol_Validate(t *testing.T) {
	...
	ex, err = action.NewExecution(
		identityset.Address(2).String()+"bbb",
		uint64(1),
		big.NewInt(0),
		uint64(0),
		big.NewInt(0),
