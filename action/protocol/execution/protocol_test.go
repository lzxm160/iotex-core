...
func TestProtocol_Handle(t *testing.T) {
	testEVM := func(t *testing.T) {
		...
		ws, err := sf.NewWorkingSet()
		require.NoError(err)
		_, err = accountutil.LoadOrCreateAccount(ws, identityset.Address(26).String(), unit.ConvertIotxToRau(1000000000))
		require.NoError(err)
		...
		selp, err := action.Sign(elp, identityset.PrivateKey(26))
		require.NoError(err)

		actionMap := make(map[string][]action.SealedEnvelope)
		actionMap[identityset.Address(26).String()] = []action.SealedEnvelope{selp}
		blk, err := bc.MintNewBlock(
			actionMap,
			testutil.TimestampNow(),
		)
		...
		exes, err := bc.GetActionsFromAddress(identityset.Address(26).String())
		require.Nil(err)
