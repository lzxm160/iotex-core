...
func readStateTotalStakingAmountFromHeight(ctx context.Context, csr CandidateStateReader, _ *iotexapi.ReadStakingDataRequest_TotalStakingAmount, height uint64) (*iotextypes.AccountMeta, uint64, error) {

	meta := iotextypes.AccountMeta{}
	meta.Address = address.StakingBucketPoolAddr
	total, err := getTotalStakedAmountFromHeight(ctx, csr, height)
	if err != nil {
		return nil, height, err
	}
	meta.Balance = total.String()
...
func getTotalStakedAmountFromHeight(ctx context.Context, csr CandidateStateReader, height uint64) (*big.Int, error) {
