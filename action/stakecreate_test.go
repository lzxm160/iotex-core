...
func TestCreateStake(t *testing.T) {
	...
	cs, err := NewCreateStake(0, "can", big.NewInt(10), 1000, true, []byte("payload"), gaslimit, gasprice)
	require.NoError(err)
	...
	require.Equal("can", cs.CandName())
	require.Equal(1000, cs.Duration())
	...
	require.Equal("can", cs2.CandName())
