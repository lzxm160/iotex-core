...
func TestSuggestGasPriceForUserAction(t *testing.T) {
	...
	blkMemDao := blockdao.NewBlockDAO(db.NewMemKVStore(), nil, sf, cfg.Chain.CompressBlock, cfg.DB)
...
func TestEstimateGasForAction(t *testing.T) {
	...
	require.NotNil(act)
	cfg := config.Default
	blkMemDao := blockdao.NewBlockDAO(db.NewMemKVStore(), nil, nil, cfg.Chain.CompressBlock, cfg.DB)
	sf, err := factory.NewFactory(cfg, factory.InMemTrieOption())
	require.NoError(err)
