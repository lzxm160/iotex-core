...
func (stx *stateTX) putIndex(pkHash hash.Hash160, ss []byte) error {
	...
	indexKey := append(AccountMaxVersionPrefix, pkHash[:]...)
	stx.cb.Put(AccountKVNameSpace, indexKey, currentVersion, "error when putting address index = %d", stx.ver)

	stateKey := append(pkHash[:], currentVersion...)
	stx.cb.Put(AccountKVNameSpace, stateKey, ss, "error when putting k = %x", pkHash)
	return nil
}
